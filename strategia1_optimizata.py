# Cod extras din Versiune 2.ipynb (Strategia 1) cu logica modificată
...
def prepare_optimization_vectors():
    ...
    BOUND_OVERRIDES = {
        ...
        "atrPeriodSl": (0.50, 2.50),          
        "multiplierPeriodSl": (0.50, 3.0),
        "sl": (0.25, 1.25),
    }
...
def compute_spread(df: pd.DataFrame, pay_fees_with_bnb: bool = False, bar_length: str = "H2") -> float:
    """
...
def optimal_strategy(params_tuple: tuple, symbol: str, start: str, end: str, bar_length: str = "H2") -> float:
    """
    ...
    try:
        ...
        short_obj = ShortParams(
            Position="Both", TP_options="Both", SL_options=sl_opt,
        ...
        dd = float(metrics.get("max_drawdown_pct", 0.0) or 0.0) / 100.0
        ...
        # Dacă apare o eroare la calcul, marcăm proba ca nerelevantă (valoare mare -> ignorată)
        print(f"Eroare la evaluarea setului de parametri: {exc}")
        return 1e6
        ...
        while attempt < max_attempts and not success:
            try:
                ...
                short_obj_opt = ShortParams(
                    Position="Both", TP_options="Both", SL_options=sl_opt_full,
                ...
                metrics = compute_performance_metrics(bt_full, initial_equity=1.0, log_returns=backtester_opt.log_return)
